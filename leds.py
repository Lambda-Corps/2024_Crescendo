...
class LEDSubsystem(Subsystem):
    ...
    def setRightSideRGB(self, red: int, green: int, blue: int ) -> None:
        for i in range(kRightSideBufferStartPoint, kRightSideBufferStartPoint + kRightSideLEDCount):
            self.ledData[i].setRGB(red, green, blue)   #  Using RGB

    def setLeftSideRGB(self, red: int, green: int, blue: int ) -> None:
        for i in range(kLeftSideBufferStartPoint, kLeftSideBufferStartPoint + kLeftSideLEDCount):
            self.ledData[i].setRGB(red, green, blue)   #  Using RGB
    ...
    def setLauncherRunwayLights(self) -> None:
        ...
        self.ledData[kLauncherStartPoint + self._RunwayLightcounter].setRGB(255, 255, 255) # Turn on new light

        # Turn off previous light
        if self._RunwayLightcounter > 0:    # special case - first light
            self.ledData[kLauncherStartPoint + self._RunwayLightcounter-1].setRGB(0,0,0) # Turn off previous light
        if self._RunwayLightcounter == 0:   #  special case - last light
            self.ledData[kLauncherStartPoint + kLauncherLEDCount-1].setRGB(0,0,0) # Turn off previous light
